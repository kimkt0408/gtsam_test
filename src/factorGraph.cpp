...
GtsamOptimizer::GtsamOptimizer(ros::NodeHandle nh) : nh_(nh) {
    ...
    nh_.param<float>("large_gps_noise_threshold", largeGpsNoiseThreshold_, 4e-1);  // 4e-2 / 9e-2 / 2023-08-22-11-19-45: 1e-1 
    nh_.param<float>("small_gps_noise_threshold", smallGpsNoiseThreshold_, 4e-4);  // 4e-4 / 2023-09-14
    
    nh_.param<float>("large_pose_covariance_threshold", largePoseCovThreshold_, 5e-2);  // 2e-2
    nh_.param<float>("small_pose_covariance_threshold", smallPoseCovThreshold_, 2e-2);  // 2e-2, 2023-09-14
...
void GtsamOptimizer::publishCurrentPose() {
    ...
    cout << "Pose covariance after gtsam:" <<poseCovariance_(3,3) << " " << poseCovariance_(4,4) << "====" << poseCovThreshold_ << endl;
