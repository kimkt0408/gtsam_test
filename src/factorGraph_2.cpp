...
GtsamOptimizer::GtsamOptimizer(ros::NodeHandle nh) : nh_(nh) {
    ...
    // ROS publishers
    posePub_ = nh.advertise<geometry_msgs::PoseStamped>("gtsam/optimized_pose_2", 10);
    trajectoryPub_ = nh.advertise<visualization_msgs::MarkerArray>("gtsam/trajectory_2", 10);
    mapCloudPub_ = nh.advertise<visualization_msgs::MarkerArray>("gtsam/mapCloud_2", 10);

    // ROS publishers (convert rostopics to map frame)
    gpsMapPub_ = nh.advertise<nav_msgs::Odometry>("gtsam/gpsOnMap_2", 10);
    odomMapPub_ = nh.advertise<nav_msgs::Odometry>("gtsam/odomOnMap_2", 10);
...
void GtsamOptimizer::addGPSFactor() {    
    ...
    cout << "Pose covariance before gtsam2:" <<poseCovariance_(3,3) << " " << poseCovariance_(4,4) << "====" << poseCovThreshold_ << endl;
    ...
    while (!gpsQueue_.empty()) {
        ...
        cout << "GPS Noise: " << noise_x << " " << noise_y << " " << noise_z << " " << (noise_x > gpsNoiseThreshold_ || noise_y > gpsNoiseThreshold_) << endl;

        if (noise_x > gpsNoiseThreshold_ || noise_y > gpsNoiseThreshold_){
        // if (noise_x > gpsNoiseThreshold_ && noise_y > gpsNoiseThreshold_){
            cout << "Too large GPS Noise!!!" << endl;
            continue;
        }
...
void GtsamOptimizer::publishCurrentPose() {
    ...
    cout << "Pose covariance after gtsam2:" <<poseCovariance_(3,3) << " " << poseCovariance_(4,4) << "====" << poseCovThreshold_ << endl;
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "gtsam_optimizer2");
    ros::NodeHandle nh;
